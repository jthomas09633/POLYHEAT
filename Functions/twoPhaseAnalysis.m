...
% iniitializing the result
    ...
    % melt peak position
    [melt_peak,first_der,sec_der] = prep2PhaseData(localSegement);
    result.meltTemp = localSegment(melt_peak,1);
    ...
    result.startOfTg = localSegement(best_solid_state_fit.length,1);
    ...
    [best_baseline,best_rubbery_fit,best_liquid_fit] = bestBaseline(localSegment,...
        ...
        full_melt_fits_Li2Ru,full_melt_fitsRu2Li);
    ...
    [heat_of_fusion_total,heat_of_fusion_first_contact,heat_of_fusion_secondary] = ...
        heatOfFusion(localSegment,best_baseline);
    ...
    result.fictveTemp = fictive_temp;
